...
if(!sistem || sistem == null || sistem == undefined || sistem == 'kapat') return;
  ...
  if(!isim) return message.channel.send('Seni Kayıt Etmem İçin Bir İsim Girmelisin : `e!kayıtol Beyza 20`')
if(!yas) return message.channel.send('Yaş İçin En Az 1 Basamak Belirtebilirsin. Örnek: `e!kayıtol Beyza 20`')
  if(yas.length >= 3) return message.channel.send('Yaş İçin En Fazla 2 Basamak Belirtebilirsin. Örnek: `e!kayıtol Beyza 20`')
  if(isNaN(yas)) return message.channel.send('Yaş İçin Sayı Girmelisin. Örnek: `e!kayıtol Beyza 20`')
  
  
  message.member.setNickname(`${isim} | ${yas}`)
  message.channel.send(`<@${message.author.id}> Kaydınız Oluşturuldu.`)
  setTimeout(() => {
